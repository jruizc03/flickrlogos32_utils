...
valset_images = []
trainvalset_annots = []
with open('trainset.relpaths.txt', 'r') as infile:
    for line in infile:
        clean_line = line.replace('\n', '')
        trainset_images.append('/home/julio/wdir/datasets/flickrlogos32/FlickrLogos-v2/'+clean_line)

with open('valset-logosonly.relpaths.txt', 'r') as infile:
    for line in infile:
        clean_line = line.replace('\n', '')
        valset_images.append('/home/julio/wdir/datasets/flickrlogos32/FlickrLogos-v2/'+clean_line)

trainvalset_images = trainset_images + valset_images

#Store the location of the image files
for imagefile in trainvalset_images:
    temp = imagefile.replace('jpg', 'masks', 1)
    temp = temp + '.bboxes.txt'
    trainvalset_annots.append(temp)

print('Trainvalset number of annotations = ' + str(len(trainvalset_annots)))
#Retrieve the x1 y1 x2 y2 for each of the annotation files
for i in range(len(trainvalset_annots)):
    with open(trainvalset_annots[i], 'r') as infile:
        for line in infile:
            if hasNumbers(line):
                numbers = line.split(' ')
                ...
                subdirectories = trainvalset_annots[i].split('/')
...
for i in range(len(imagefilenames)):
    with open('flickr_logos_32_dataset_training_set_annotation.txt', 'a') as infile:
